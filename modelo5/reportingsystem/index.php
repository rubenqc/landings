<?php 
	...
	$query = $mysqli->query("SELECT nombre,apellido,dni,celular,correo,mensaje,registro FROM tabla ORDER BY registro DESC LIMIT 62755");
	...
	while ($row = $query->fetch_row()) {
		$tbody.='<tr><td>'.$row[6].'</td><td>'.ucwords(strtolower($row[0])).'</td><td>'.ucwords(strtolower($row[1])).'</td><td align="center">'.$row[2].'</td><td>'.$row[3].'</td><td>'.strtolower($row[4]).'</td><td>'.strtolower($row[5]).'</td></tr>';
	}
...
<body>
	<header class="page-header">
		<div class="container">
			<h1>Agentes de Seguridad - Reporting System</h1>			
		</div>
	</header>
